...
// Define header and its length (without a null terminator)
#define HEADER "securbyte_1.1"
...
int encrypt_file(const char *filename, const char *password)
{
    ...
    // Write the header, IV, and the encrypted buffer into the file
    size_t total_length = HEADER_LENGTH + 16 + length;
    uint8_t *output_buffer = malloc(total_length);
    ...
    memcpy(output_buffer + HEADER_LENGTH, iv, 16);
    memcpy(output_buffer + HEADER_LENGTH + 16, padded_buffer, length);
    free(padded_buffer);
    int result = write_file(filename, output_buffer, total_length);
    free(output_buffer);
    return result;
}
...
int decrypt_file(const char *filename, const char *password)
{
    ...
    // Check that the length is at least HEADER_LENGTH + 16 bytes
    ...
    // Verify the header
    if (memcmp(buffer, HEADER, HEADER_LENGTH) != 0)
    {
        free(buffer);
        printf("\nError: This file was not encrypted using SecurByte 1.1 or may have been modified.\n");
        return -1;
    }
    // Hash the password using SHA-256
    uint8_t key[32];
    ...
    uint8_t iv[16];
    memcpy(iv, buffer + HEADER_LENGTH, 16);
    // Initialize the AES context with the key and the IV
    struct AES_ctx aes_ctx;
    AES_init_ctx_iv(&aes_ctx, key, iv);
    // Decrypt the buffer using AES-256-CBC; the cipher text starts after HEADER_LENGTH + 16
    size_t cipher_length = length - HEADER_LENGTH - 16;
    AES_CBC_decrypt_buffer(&aes_ctx, buffer + HEADER_LENGTH + 16, cipher_length);
    // Unpad the buffer
    size_t unpadded_length = cipher_length;
    uint8_t *unpadded_buffer = unpad_buffer(buffer + HEADER_LENGTH + 16, &unpadded_length);
    if (unpadded_buffer == NULL)
    {
        free(buffer);
        return -1;
    ...
    int result = write_file(filename, unpadded_buffer, unpadded_length);
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2 || argc > 6)
    {
        printf("\nSecurByte 1.1 (x64) : (c) 8gudbits - All rights reserved.\n");
